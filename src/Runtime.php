...
final class Runtime {
     ...
     * Internal objects generally use a custom structure which cannot be copied by value safely, PHP currently lacks the
     *  mechanics to do this (without serialization) and so only objects that do not use a custom structure may be shared.
     * Some internal objects do not use a custom structure, for example {@see Event} and so may be shared.
     * Closures are a special kind of internal object and support being copied by value, and so may be shared.
     * Channels are central to writing parallel code and support concurrent access and execution by necessity,
     *  and so may be shared.
     * WARNING: A user class that extends an internal class may use a custom structure as defined by the internal class,
     *  in which case they cannot be copied by value safely, and so may not be shared.
     *
     * WARNING: The return {@see Future} must not be ignored when the task contains a return or throw statement.
     *
     * @param \Closure $task
     * @param array    $argv
     *
